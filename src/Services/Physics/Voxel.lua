...
local instancify = packages("instancify")
local draw = packages("draw")

local Substance = import("shared/Substance")
local geometry = import("shared/Geometry")
local config = import("shared/Config")

local debugSimplify = true
...
return function(worker: Worker)
	function worker:Cut(mainParts, cutParts)
	...
	instancify.set(worker, worker.Instance, true)
